...
function efectoHabilidades(){
  ...
  if(distancia_skills>=300 && entro==false){
      ...
      const intervalTypescript = setInterval(function(){
        pintarBarra(typescript, 8, 2, intervalTypescript);
    },100);
      const intervalReact = setInterval(function(){
        pintarBarra(react, 13, 3, intervalReact);
    },100);
    const intervalPython = setInterval(function(){
      pintarBarra(python, 8, 4, intervalPython);
  },100);
  const intervalAngular = setInterval(function(){
    pintarBarra(angular, 7, 5, intervalAngular);
},100);
  }
}
